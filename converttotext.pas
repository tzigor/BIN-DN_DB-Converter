...
begin
  ...
  begin
    ...
    try
      repeat;                                                           {следующую запись в журнале пока не нашли                                       }
        b:= 0;
        ...
        if b = $C0 then begin
          ...
          if  CurrentRecord.N > 0 then begin
              ...
              DateTime:= EncodeDateTime(StrToInt(IntToHex(CurrentRecord.Data[0])), StrToInt(IntToHex(CurrentRecord.Data[1])), StrToInt(IntToHex(CurrentRecord.Data[2])),
               ...
               Writeln(FS, emptystr);
               s:=       IntToHex(CurrentRecord.Data[2], 2) + DateSeparator + IntToHex(CurrentRecord.Data[1], 2) + DateSeparator + IntToHex(CurrentRecord.Data[0], 2) + ' ' +
                        IntToHex(CurrentRecord.Data[3], 2) + TimeSeparator + IntToHex(CurrentRecord.Data[4], 2) + TimeSeparator + IntToHex(CurrentRecord.Data[5], 2);
              ...
              case CurrentRecord.Cmd of
                ...
                02: {$REGION ' запись статического замера '}
                    begin
                      ...
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Статический замер');
                ...
                07: {$REGION ' запись количества слов синхронизирующей преамбулы '}
                    begin
                      ...
                      s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[06]) + #09#09 + 'количество слов синхронизирующей преамбулы';
                    end;
                    {$ENDREGION}
                ...
                08: {$REGION ' запись типа модуляции '}
                    begin
                      ...
                      s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[06]) + #09 + 'тип модуляции';
                      Writeln(FS, s);

                      {Buf[07]: источник изменения (0..2). }
                      s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[07]) + #09 + 'источник изменения';
                    end;
                    {$ENDREGION}
                ...
                09: {$REGION ' запись индекса таблицы частота/скорость '}
                    begin
                      ...
                      s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[06]) + #09#09 + 'индекс таблицы';
                      Writeln(FS, s);

                      {Buf[07]: источник изменения (0..2). }
                      s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[07]) + #09#09 + 'источник изменения (0..2)';
                    end;
                    {$ENDREGION}
                ...
                10: {$REGION ' запись разрешения/запрета ответа при изменении режимов работы '}
                    begin
                      ...
                      s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[06]) + #09#09 + 'код разрешения/запрета ответа при изменении режимов работы';
                    end;
                    {$ENDREGION}
                ...
                11: {$REGION ' запись разрешения/запрета изменения режимов работы '}
                    begin
                      ...
                      s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[06]) + #09#09 + 'код разрешения/запрета изменения режимов работы';
                    end;
                    {$ENDREGION}
                ...
                33: {$REGION ' запись широты местности '}
                    begin
                     ...
                     // s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[06]) + '°';

                      // w:= MakeWord(CurrentRecord.Data[07],CurrentRecord.Data[06]);
                     //// Move(CurrentRecord.Data[06], w, 2);
                      // s:= #09#09#09#09 + Format('%.2f', [w / 100]) + '°';

                        smi:= FillWord(CurrentRecord.Data[07],CurrentRecord.Data[06]);
                ...
                34: {$REGION ' запись модуля вектора магнитного поля местности '}
                    begin
                      ...
                      Move(CurrentRecord.Data[06], i, 4);
                ...
                35: {$REGION ' запись магнитного наклонения местности '}
                    begin
                     ...
                     // Move(CurrentRecord.Data[06], w, 2);
                     // d:= w / 100;
                      Move(CurrentRecord.Data[06], smi, 2);
                      d:= smi / 100;
                ...
                40: {$REGION ' запись при выключении питания - другой формат ударов (бывш. 01) '}
                    begin
                      ...
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Выключение питания (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                        ...
                        00: {$REGION ' версия №00 '}
                            begin
                              ...
                              Writeln(FS, s);
                                ...
                                {байт 48, бит 4.}
                                if ((CurrentRecord.Data[48] and 16) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: ошибка контрольного кода БЭКОЗУ';
                                  Writeln(FS, s);
                                end;

                                {байт 48, бит 5.}
                                if ((CurrentRecord.Data[48] and 32) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: ошибка записи в БЭКАП при выключении';
                                  Writeln(FS, s);
                                end;
                                ...
                                {байт 48, биты 6 и 7.}
                                w:= 0;
                                if ((CurrentRecord.Data[48] and  64) <> 0) then w:= w + 1;
                                if ((CurrentRecord.Data[48] and 128) <> 0) then w:= w + 2;
                                case w of
                        ...
                        02: {$REGION ' версия №02 '}
                            begin
                              ...
                              Writeln(FS, s);
                                ...
                                {байт 47, бит 7.}
                                if ((CurrentRecord.Data[47] and 128) <> 0) then begin
                                ...
                                {байт 48, бит 1.}
                                if ((CurrentRecord.Data[48] and 2) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: остаток текущего сектора не пригоден для записи';
                                  Writeln(FS, s);
                                end;

                                {байт 48, бит 2.}
                                if ((CurrentRecord.Data[48] and 4) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: не найдено начало последней записи';
                                  Writeln(FS, s);
                                end;

                                {байт 48, бит 3.}
                                if ((CurrentRecord.Data[48] and 8) <> 0) then begin
                        ...
                        03: {$REGION ' версия №03 '}
                            begin
                              ...
                              Writeln(FS, s);
                                ...
                                {байт 28, бит 0.}
                                if ((CurrentRecord.Data[28] and 1) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: состояние массива(0x1FE000) основных параметров последовательной флэш';
                                  Writeln(FS, s);
                                end;

                                {байт 28, бит 1.}
                                if ((CurrentRecord.Data[28] and 2) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: состояние массива(0x1FF000) основных параметров последовательной флэш';
                                  Writeln(FS, s);
                                end;

                                {байт 28, бит 2.}
                                if ((CurrentRecord.Data[28] and 4) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: ошибка CRC в записи журнала';
                                  Writeln(FS, s);
                                end;

                                {байт 28, бит 3.}
                                if ((CurrentRecord.Data[28] and 8) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: состояние часов 8586';
                                  Writeln(FS, s);
                                end;

                                {байт 28, бит 4.}
                                if ((CurrentRecord.Data[28] and 16) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: наличие часов 8565';
                                  Writeln(FS, s);
                                end;

                                {байт 28, бит 5.}
                                if ((CurrentRecord.Data[28] and 32) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: ошибка контрольного кода БЭКАП памяти';
                                  Writeln(FS, s);
                                end;

                                {байт 28, бит 6.}
                                if ((CurrentRecord.Data[28] and 64) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: ошибка записи в БЭКАП при выключении';
                                  Writeln(FS, s);
                                end;

                                {байт 28, бит 7.}
                                if ((CurrentRecord.Data[28] and 128) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: последняя запись отличается от "Выключение питания"';
                                  Writeln(FS, s);
                                end;
                                ...
                                {байт 29, бит 1.}
                                if ((CurrentRecord.Data[29] and 2) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: остаток текущего сектора не пригоден для записи';
                                  Writeln(FS, s);
                                end;

                                {байт 29, бит 2.}
                                if ((CurrentRecord.Data[29] and 4) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: не найдено начало последней записи';
                                  Writeln(FS, s);
                                end;

                                {байт 29, бит 3.}
                                if ((CurrentRecord.Data[29] and 8) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: ошибка выключения питания - более 2-х минут на аккумуляторе';
                                  Writeln(FS, s);
                                end;

                                {байт 29, бит 4.}
                                if ((CurrentRecord.Data[29] and 16) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: ошибка контрольного кода БЭКОЗУ';
                                  Writeln(FS, s);
                                end;

                                {байт 29, бит 5.}
                                if ((CurrentRecord.Data[29] and 32) <> 0) then begin
                                  s:= #09#09#09#09#09 + '- ошибка: ошибка записи в БЭКОЗУ при выключении';
                                  Writeln(FS, s);
                                end;
                                ...
                                {байт 29, биты 6 и 7.}
                                w:= 0;
                                if ((CurrentRecord.Data[29] and  64) <> 0) then w:= w + 1;
                                if ((CurrentRecord.Data[29] and 128) <> 0) then w:= w + 2;
                                case w of
                                  0 : s:= #09#09#09#09#09 + '- напряжение аккумулятора под нагрузкой U >= 3.6 В';
                                  1 : s:= #09#09#09#09#09 + '- напряжение аккумулятора под нагрузкой 3.6V > U >= 3.4 В';
                                  2,
    ...
    ;
                        ...
                        08:  {$REGION ' версия №08 '}
                            begin
                             ...
                             //s:=  #09#09#09#09 + IntToHex(CurrentRecord.Data[58], 2) + DateSeparator + IntToHex(CurrentRecord.Data[57], 2) + DateSeparator + IntToHex(CurrentRecord.Data[56], 2) + ' ' +
                             //     IntToHex(CurrentRecord.Data[59], 2) + TimeSeparator + IntToHex(CurrentRecord.Data[60], 2) + TimeSeparator + IntToHex(CurrentRecord.Data[61], 2) +
                ...
                41: {$REGION ' запись параметров передачи силового модуля - новый формат ударов (бывш. 06) '}
                    begin
                      ...
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Параметры передатчика (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                      ...
                      case CurrentRecord.Data[6] of                                   {в новом формате записей в байте [6] всегда лежит № версии                      }
                        ...
                        10: {$REGION ' версия №08 MUP '}                 {Konovalov 06/06/2022}
                             begin
                              ...
                              s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[55]) + '°'#09 + 'температура МУП';
                ...
                42: {$REGION ' запись динамического замера '}
                    begin
                      ...
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Динамический замер (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                      ...
                      case CurrentRecord.Data[6] of                                   {в новом формате записей в байте [6] всегда лежит № версии                      }
                        00: {$REGION ' версия №00 '}
                            begin
                             ...
                             // s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[20]) + '°'#09+ 'температура модуля инклинометра';
                ...
                43: {$REGION ' разрешение =1 запрет =0 режима работы с силовым аккумуляторным модулем '}
                    begin
                      {Buf[00]..Buf[05]: дата и время в формате BCD.
                       ...
                       Buf[07]         : данные. }
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Работа от силового аккумулятора (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');

                      if (CurrentRecord.N = 7) then                                   {при длине 7 ещё не ввели версию записи                                         }
                      case CurrentRecord.Data[06] of
                        0 : s:= #09#09#09#09 + 'запрещено';
                        1 : s:= #09#09#09#09 + 'разрешено';
                        else
                ...
                45: {$REGION ' переключение пакетов с магнитных на гравитационные: 1 - по роторному/динамическому зениту; 0 - по статическому зениту '}
                    begin
                      {Buf[00]..Buf[05]: дата и время в формате BCD.
                       ...
                       Buf[07]         : данные. }
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Переключение пакетов с магнитных на гравитационные (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                ...
                46: {$REGION ' переключение типа модуляций манипуляцией давлением: разрешение = 1; запрет = 0 '}
                    begin
                      {Buf[00]..Buf[05]: дата и время в формате BCD.
                       ...
                       Buf[07]         : данные. }
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Переключение типа модуляций манипуляцией давлением (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                ...
                47: {$REGION ' оценка состояния энергонезависимой памяти и журнала '}
                    begin
                      ...
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Оценка состояния энергонезависимой памяти и журнала (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                      ...
                      case CurrentRecord.Data[6] of                                   {в новом формате записей в байте [6] всегда лежит № версии                      }
                        00: {$REGION ' версия №00 '}
                            begin
                              ...
                              {байт 07, бит 0.}
                              if ((CurrentRecord.Data[07] and 1) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: состояние массива(0x1FE000) основных параметров последовательной флэш';
                                Writeln(FS, s);
                              end;

                              {байт 07, бит 1.}
                              if ((CurrentRecord.Data[07] and 2) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: состояние массива(0x1FF000) основных параметров последовательной флэш';
                                Writeln(FS, s);
                              end;

                              {байт 07, бит 2.}
                              if ((CurrentRecord.Data[07] and 4) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: ошибка CRC в записи журнала';
                                Writeln(FS, s);
                              end;

                              {байт 07, бит 3.}
                              if ((CurrentRecord.Data[07] and 8) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: состояние часов 8586';
                                Writeln(FS, s);
                              end;

                              {байт 07, бит 4.}
                              if ((CurrentRecord.Data[07] and 16) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: наличие часов 8565';
                                Writeln(FS, s);
                              end;

                              {байт 07, бит 5.}
                              if ((CurrentRecord.Data[07] and 32) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: ошибка контрольного кода БЭКАП памяти';
                                Writeln(FS, s);
                              end;

                              {байт 07, бит 6.}
                              if ((CurrentRecord.Data[07] and 64) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: ошибка записи в БЭКАП при выключении';
                                Writeln(FS, s);
                              end;

                              {байт 07, бит 7.}
                              if ((CurrentRecord.Data[07] and 128) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: последняя запись отличается от "Выключение питания"';
                                Writeln(FS, s);
                              end;
                              ...
                              {байт 08, бит 1.}
                              if ((CurrentRecord.Data[08] and 2) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: остаток текущего сектора непригоден для записи';
                                Writeln(FS, s);
                              end;

                              {байт 08, бит 2.}
                              if ((CurrentRecord.Data[08] and 4) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: не найдено начало последней записи';
                                Writeln(FS, s);
                              end;

                              {байт 08, бит 3.}
                              if ((CurrentRecord.Data[08] and 8) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: ошибка выключения питания - более 2-х минут на аккумуляторе';
                                Writeln(FS, s);
                              end;

                              {байт 08, бит 4.}
                              if ((CurrentRecord.Data[08] and 16) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: ошибка контрольного кода БЭКОЗУ';
                                Writeln(FS, s);
                              end;

                              {байт 08, бит 5.}
                              if ((CurrentRecord.Data[08] and 32) <> 0) then begin
                                s:= #09#09#09#09#09 + '- ошибка: ошибка записи в БЭКОЗУ при выключении';
                                Writeln(FS, s);
                              end;
                              ...
                              {байт 08, биты 6 и 7.}
                              w:= 0;
                              if ((CurrentRecord.Data[08] and  64) <> 0) then w:= w + 1;
                              if ((CurrentRecord.Data[08] and 128) <> 0) then w:= w + 2;
                              case w of
                                0 : s:= #09#09#09#09#09 + '- напряжение аккумулятора под нагрузкой U >= 3.6 В';
                                1 : s:= #09#09#09#09#09 + '- напряжение аккумулятора под нагрузкой 3.6V > U >= 3.4 В';
                                2,
                ...
                48: {$REGION ' команда даунлинк режима переключения пакетов '}
                    begin
                      {Buf[00]..Buf[05]: дата и время в формате BCD.
                       ...
                       Buf[07]         : данные. }
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Команда даунлинк режима переключения пакетов (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                ...
                49: {$REGION ' нарушение условий измерения статики '}
                    begin
                      {Buf[00]..Buf[05]: дата и время в формате BCD.
                       ...
                       Buf[07]         : данные. }
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Нарушение условий измерения статики (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                      ...
                      case CurrentRecord.Data[6] of                                   {в новом формате записей в байте [6] всегда лежит № версии                      }
                        00: {$REGION ' версия №00 '}
                            begin
                              ...
                              s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[07]) + ' c'#09+ 'количество секунд с момента начала 30-ти секундной паузы';
                            end;
                            {$ENDREGION}
                ...
                51: {$REGION ' изменены настройки LTB '}
                    begin
                      ...
                      Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Изменены настройки LTB (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                      ...
                      case CurrentRecord.Data[6] of                                   {в новом формате записей в байте [6] всегда лежит № версии                      }
                        0 : {$REGION ' версия №00 '}
                            begin
                              {Buf[07]: . кол-во параметров LTB}
                              s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[07]) + #09 + 'количество параметров LTB';
                            end;
                            {$ENDREGION}
                ...
                52: {$REGION ' перезапсиь настроек МИ из LTB-модема '}
                    begin
                      ...
                      case CurrentRecord.Data[6] of                                   {в новом формате записей в байте [6] всегда лежит № версии                      }
                        0 : {$REGION ' версия №00 '}
                            begin
                              s:= 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Перезапись настроек МИ из LTB модема (  вер. ' + IntToStr(CurrentRecord.Data[6]) + ')';
                            end;
                            {$ENDREGION}
                ...
                53: {$REGION ' запись о параметрах старта программы'}
                    begin
                     ...
                     //Writeln(FS, 'Запись о параметрах старта программы (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');

                      s:= 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Запись о параметрах старта программы (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')' + ':    ';
                ...
                54: {$REGION ' - запись об исправлении времени ударов'}
                    begin
                       ...
                       Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Запись об исправлении времени ударов (вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                      ...
                      case CurrentRecord.Data[6] of                                   {в новом формате записей в байте [6] всегда лежит № версии                      }
                        00: {$REGION ' версия №00 '}
                            begin
                              {Buf[07]: . кол-во параметров LTB}
                              s:= #09#09#09#09 + IntToStr(CurrentRecord.Data[07]) + #09 + 'код исправленного времени';
                ...
                100: {$REGION ' - запись Вводная информация - месторождение'}
                    ...
                    Writeln(FS, 'Cmd: ' + IntToStr(CurrentRecord.Cmd) + '  Вводная информация(вер. ' + IntToStr(CurrentRecord.Data[6]) + ')');
                    // Move(00, Infob[00], Length(Infob));

                        SetLength(Infob, CurrentRecord.N);
                        FillChar (Infob[00], CurrentRecord.N, $00);

                      case CurrentRecord.Data[6] of                                   {в новом формате записей в байте [6] всегда лежит № версии                      }
                        00: {$REGION ' версия №00 '}
                            begin
                              Move(CurrentRecord.Data[7], Infob[00], CurrentRecord.N);
